...
class MTTopic: NSObject {
    ...
    init(dic:JSON){
        ...
        if !dic["top_cmt"].isEmpty{
            self.top_cmt = MTComment(dic: dic["top_cmt"][0])
        }
        
        let topic = self.text as NSString
        let maxSize = CGSizeMake(UIScreen.mainScreen().bounds.width - 30,CGFloat(MAXFLOAT))
        let textH = topic.boundingRectWithSize(maxSize, options: .UsesLineFragmentOrigin, attributes: [NSFontAttributeName : UIFont.systemFontOfSize(14)], context: nil).size.height
        // 只有文字时的高度
        self.cellHeight = textH + MTTopicCellTextY + 2 * MTMargin + MTCellBottomBtnH
        
        // 图片的宽度和高度 （包括视频和音频）
        let width = maxSize.width
        var height = self.height * width / self.width
        let top = MTTopicCellTextY  + textH + MTMargin
        ...
        if self.type != .Word {
            ...
            if self.type == .Picture{
                //  当为图片 并且高度太高时
                if height > MTCellPictureMaxImageH{
                    height = MTCellPictureModifiedH
                    self.isBigImage = true
                }
            }
            // 只要不是图片都有frame
            self.pictureFrame = CGRect(x: MTMargin, y:top, width: width, height: height)
            self.cellHeight += height + MTMargin
        }
        ...
        if let comment = self.top_cmt{
            ...
            self.cellHeight += MTHotCommentTitleH + commentH + MTMargin
        }
    }
    ...
    class func topics(array:[JSON]) -> NSMutableArray{
        ...
        for item in array{
            arrayM.addObject(MTTopic.init(dic: item))
        }
